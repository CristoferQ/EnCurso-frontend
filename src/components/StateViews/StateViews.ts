...
export function createErrorStateElement(
  message: string = 'Ocurrió un problema al cargar los cursos.',
  onRetry?: () => void,
): HTMLElement {
  ...
  container.innerHTML = `
    <h3 class="text-lg font-bold text-red-500 mb-2 uppercase">¡Ups! Ocurrió un problema al cargar los cursos.</h3>
...
export function createEmptyStateElement(): HTMLElement {
  ...
  container.innerHTML = `
    <p class="text-base font-bold uppercase">No hay cursos disponibles por el momento. ¡Vuelve pronto!</p>
  `;
  return container;
}
